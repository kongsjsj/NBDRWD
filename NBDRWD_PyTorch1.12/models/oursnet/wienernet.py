...
from models.commonblocks import (p2o_new, csum1, Conv, Deconv, ResidualBlock)
...
class NBDRWD(nn.Module):
    ...
    def forward(self, x, k):
        ...
        for i in range(x_feature.shape[1]):
            blur_feature_ch = x_feature[:, i:i + 1, :, :]
            bfc_img = variable_to_cv2_image(blur_feature_ch)
            clear_feature_ch = wiener_deblur(blur_feature_ch, k)
            clear_features[:, i:i + 1, :, :] = clear_feature_ch[:, :, ks[1]:-ks[1], ks[0]:-ks[0]]
...
def wiener_deblur(inputs, kernel):
    ...
    """
    w, h = inputs.shape[-2:]
    fk = p2o_new(kernel, (w, h))  # F(k)  # PyTorch 1.8.1 and latest version
    fkc = fk.conj()
    fk_fk = torch.abs(fk) ** 2
    fk2 = fk_fk.clone().detach().to(dtype=torch.complex64)  # F(k)_bar * F(k)
    snr = wiener_filter_para(inputs)
    fk2 = csum1(fk2, snr)
    fkc_fy = fkc * PF.fft2(inputs)
    fkc_fy_div_fk2 = fkc_fy / fk2
    outputs = PF.ifft2(fkc_fy_div_fk2).real
